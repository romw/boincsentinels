...
void CDlgEventLog::CreateControls()
{
    ...
    m_pMessagesCopyCtrl = new wxBitmapButton( itemDialog1, wxID_COPY, wxBitmap(copy_xpm), wxDefaultPosition, wxDefaultSize, wxBU_AUTODRAW );
...
void CDlgEventLog::PopulateMessagesFilter()
{
    ...
    pState->EnumerateProjects(m_hHost, oProjects);
...
void CDlgEventLog::PopulateMessages()
{
    ...
    pState->EnumerateMessages(m_hHost, 0, oMessages);
    for (std::vector<BSLMESSAGE>::iterator iter = oMessages.begin(); iter != oMessages.end(); iter++)
    {
        ...
        item.SetId(m_uiNextItemID);
        item.SetColumn(0);
        item.SetText(wxEmptyString);
        m_pMessagesListViewCtrl->InsertItem(item);
        m_pMessagesListViewCtrl->SetItemPtrData(m_uiNextItemID, (wxUIntPtr)oMessage.GetMessageHandle());

        UpdateMessage(m_uiNextItemID, oMessage);
    ...
    //m_pMessagesListViewCtrl->SetColumnWidth(COLUMN_PROJECT, wxLIST_AUTOSIZE);
...
void CDlgEventLog::OnMessagesAdd(CBSLMessageEvent& event)
{
    ...
    if (BSLERR_SUCCESS == event.GetMessage(&oMessage))
    {
        ...
        item.SetText(wxEmptyString);
        m_pMessagesListViewCtrl->InsertItem(item);
        m_pMessagesListViewCtrl->SetItemPtrData(m_uiNextItemID, (wxUIntPtr)event.GetMessageHandle());
