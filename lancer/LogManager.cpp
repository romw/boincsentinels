...
wxLogStderr()
{
    ...
    if (wxFileExists(wxT("BOINC-Sentinels-Lancer.log")))
    {
        wxRenameFile(wxT("BOINC-Sentinels-Lancer.log"), wxT("BOINC-Sentinels-Lancer.bak"), true);
    }

    m_pFile = new wxFFile(wxT("BOINC-Sentinels-Lancer.log"), wxT("a"));
    m_fp = m_pFile->fp();
}
...
void CLogManager::DoLogText(const wxString& msg)
{
    ...
    if (m_pFile->Length() > 1024 * 1024 * 8)
    {
        ...
        m_pFile->Open(wxT("BOINC-Sentinels-Lancer.log"), wxT("a"));
        m_fp = m_pFile->fp();
    }
