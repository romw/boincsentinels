...
#include "EnumerateTaskInstancesTest.h"

CPPUNIT_TEST_SUITE_REGISTRATION(EnumerateTaskInstancesTestCase);

void EnumerateTaskInstancesTestCase::NullHandle()
{
    ...
    pClient->OnInit();
    rc = pClient->EnumerateTaskInstances(hHost, oTaskInstances);
    pClient->OnExit();
...
void EnumerateTaskInstancesTestCase::InvalidHandle()
{
    BSLERRCODE rc = BSLERR_FAILURE;
    CBSLClient* pClient = new CBSLClient();
...
void EnumerateTaskInstancesTestCase::EmptyDataSet()
{
    ...
    rcEnumerate = pClient->EnumerateTaskInstances(hHost, oTaskInstances, false);
    ...
    CPPUNIT_ASSERT(0 == oTaskInstances.size());
}
