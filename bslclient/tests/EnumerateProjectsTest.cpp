...
#include "EnumerateProjectsTest.h"

CPPUNIT_TEST_SUITE_REGISTRATION(EnumerateProjectsTestCase);

void EnumerateProjectsTestCase::NullHandle()
{
    ...
    pClient->OnInit();
    rc = pClient->EnumerateProjects(hHost, oProjects);
    pClient->OnExit();
...
void EnumerateProjectsTestCase::InvalidHandle()
{
    BSLERRCODE rc = BSLERR_FAILURE;
    CBSLClient* pClient = new CBSLClient();
...
void EnumerateProjectsTestCase::EmptyDataSet()
{
    ...
    rcEnumerate = pClient->EnumerateApps(hHost, oProjects);
    ...
    CPPUNIT_ASSERT(0 == oProjects.size());
}
