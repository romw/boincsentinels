...
#include "EnumerateHostsTest.h"

CPPUNIT_TEST_SUITE_REGISTRATION(EnumerateHostsTestCase);

void EnumerateHostsTestCase::EmptyDataSet()
{
    ...
    pClient->OnInit();
    rc = pClient->EnumerateHosts(oHosts);
    pClient->OnExit();

    delete pClient;

    CPPUNIT_ASSERT(pClient != NULL);
    CPPUNIT_ASSERT(BSLERR_SUCCESS == rc);
}

void EnumerateHostsTestCase::PopulatedDataSet()
{
    ...
    rcEnumerate = pClient->EnumerateHosts(oHosts);
