...
#include "EnumerateTasksTest.h"

CPPUNIT_TEST_SUITE_REGISTRATION(EnumerateTasksTestCase);

void EnumerateTasksTestCase::NullHandle()
{
    BSLERRCODE rc = BSLERR_FAILURE;
    CBSLClient* pClient = new CBSLClient();
    BSLHOST hHost = NULL;
    std::vector<BSLTASK> oTasks;

    pClient->OnInit();
    rc = pClient->EnumerateTasks(hHost, oTasks);
    pClient->OnExit();
...
void EnumerateTasksTestCase::InvalidHandle()
{
    BSLERRCODE rc = BSLERR_FAILURE;
    CBSLClient* pClient = new CBSLClient();
...
void EnumerateTasksTestCase::EmptyDataSet()
{
    ...
    rcEnumerate = pClient->EnumerateTasks(hHost, oTasks);
