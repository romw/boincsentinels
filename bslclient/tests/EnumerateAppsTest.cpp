...
#include "EnumerateAppsTest.h"

CPPUNIT_TEST_SUITE_REGISTRATION(EnumerateAppsTestCase);

void EnumerateAppsTestCase::NullHandle()
{
    BSLERRCODE rc = BSLERR_FAILURE;
    CBSLClient* pClient = new CBSLClient();
    BSLHOST hHost = NULL;
    std::vector<BSLAPP> oApps;

    pClient->OnInit();
    rc = pClient->EnumerateApps(hHost, oApps);
    pClient->OnExit();
...
void EnumerateAppsTestCase::InvalidHandle()
{
    BSLERRCODE rc = BSLERR_FAILURE;
    CBSLClient* pClient = new CBSLClient();
...
void EnumerateAppsTestCase::EmptyDataSet()
{
    ...
    rcEnumerate = pClient->EnumerateApps(hHost, oApps);
