...
#ifndef _RPCMANAGER_H_
#define _RPCMANAGER_H_

class CRPCManager: public wxObject
{
    DECLARE_DYNAMIC_CLASS(CRPCManager)
...
public:
    ...
    /// \brief Establish a connection to a BOINC client.
    /// \param[in] pHost a pointer to the host object to make a connection with.
    ...
    BSLERRCODE OnConnect(CHost* pHost);

    /// \brief Disconnect from a BOINC client.
    /// \param[in] pHost a pointer to the host object to disconnect.
    /// \retval BSLERR_SUCCESS \copybrief BSLERRCODE::BSLERR_SUCCESS
    /// \retval BSLERR_FAILURE \copybrief BSLERRCODE::BSLERR_FAILURE
    BSLERRCODE OnDisconnect(CHost* pHost);
