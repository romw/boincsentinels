...
#ifndef _RPCPTASKINSTANCEOPERATION_H_
#define _RPCPTASKINSTANCEOPERATION_H_

class CRPCTaskInstanceOperation: public CRPCProtocol
{
    DECLARE_DYNAMIC_CLASS(CRPCTaskInstanceOperation)

public:
    CRPCTaskInstanceOperation();
    CRPCTaskInstanceOperation(const wxChar* pszRPCName, wxUint32 uiRPCNameLength);
    virtual ~CRPCTaskInstanceOperation();
    ...
    /// \param[in] pTaskInstance the task instance to perform the operation on.
    ...
    virtual BSLERRCODE Execute(CHost* pHost, CRPCConnection::CONNECTIONTYPE type, CBSLProject* pProject, CBSLTaskInstance* pTaskInstance);
