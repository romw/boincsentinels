...
#ifndef _RPCSYNCNOTIFICATIONS_H_
#define _RPCSYNCNOTIFICATIONS_H_

class CRPCSyncNotifications: public CRPCProtocol
{
    DECLARE_DYNAMIC_CLASS(CRPCSyncNotifications)

public:
    CRPCSyncNotifications();
    ~CRPCSyncNotifications();

    /// \brief Synchronize the notifications with the BOINC client.
    /// \param[in] pHost the host to issue the RPC against.
    /// \param[in] type the type of RPC connection to use.
    /// \param[in] uiSequenceNumber the sequence number of the last notification received.
