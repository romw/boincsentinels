...
void  MD5Final(MD5_CTX* ctx, unsigned char digest[MD5_HASHBYTES]);
...
wxString wxMD5::GetMD5(const wxString &string)
{
    ...
    std::string buffer = (const char*)string.mb_str(wxConvUTF8);

    MD5Init(&ctx);
    MD5Update(&ctx, (unsigned char const*)buffer.c_str(), (unsigned int)buffer.size());
    MD5End(&ctx, tmp);
